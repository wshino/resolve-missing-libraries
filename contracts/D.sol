...
import { ICharterAccount } from "./demo/ICharter.sol";
import { IEmailRecoveryModule } from "email-recovery/src/interfaces/IEmailRecoveryModule.sol";
import { EmailRecoveryManager } from "email-recovery/src/EmailRecoveryManager.sol";
import { GuardianManager } from "email-recovery/src/GuardianManager.sol";
 ...
 * validator
 */
contract EmailRecoveryModule is EmailRecoveryManager, IEmailRecoveryModule {
    /*´:°•.°+.*•´.*:˚.°*.˚•´.°:°•.°•.*•´.*:˚.°*.˚•´.°:°•.°+.*•´.*:*/
    /*                    CONSTANTS & STORAGE                     */
    /*.•°:°.´+˚.*°.˚:*.´•*.+°.•°:´*.´•*.•°.•°:°.´:•˚°.*°.˚:*.´+°.•*/
        ...
        EmailRecoveryManager(verifier, dkimRegistry, emailAuthImpl, subjectHandler)
    {
        ...
        selector = _selector; //TODO: check if selector is valid
    }
    ...
    function Init(bytes calldata data) external {
        ...
        if (
            !ICharterAccount(msg.sender).isModule(address(this))
        ) {
    ...
    function recover(address account, bytes calldata recoveryData) internal override {
        (address validatoraddr, bytes memory recoveryCalldata) =
            abi.decode(recoveryData, (address, bytes));

        if (validatoraddr == address(0)) {
            revert InvalidValidator(validatoraddr);
        }
        ...
        ICharterAccount(account).executeFromModule(account, 0, recoveryCalldata);

        emit RecoveryExecuted(account, validatoraddr);
    }
